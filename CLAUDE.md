...
- **Horarios are "jornadas" with several cursos, not one row per curso.** `model/Jornada.java` is the aggregate root: one row per (ciclo, diaSemana, turno) — enforced by the service (`HorarioService.guardarJornada`, find-or-create), not a DB constraint — holding `horaInicio`/`horaFin` and a `List<Horario>` (cascade ALL, orphanRemoval). `Horario` is now "one curso within a jornada" (`curso`, `aula`, `jornada` FK) and **no longer has its own ciclo/turno/diaSemana/horaInicio/horaFin columns** — those were dropped from the `horarios` table during the migration to `jornadas`. `Horario` keeps read-only delegate getters (`getDiaSemana()`, `getTurno()`, `getHoraInicio()`, `getHoraFin()`, `getCiclo()`) that forward to `jornada.getX()`, specifically so `AsistenciaServiceImpl` and `templates/asistencias/escanear.html` — which read `horario.diaSemana`/`horario.turno`/etc. — didn't need to change. The old "cruce de horario" (time-overlap) validation was removed: it no longer applies, since all cursos in a jornada intentionally share the same time block and there's at most one jornada per ciclo+día+turno. `/horarios` shows the full week (6 días × 3 turnos) at once, no turno filter — `HorarioController.nuevo` decides between "crear jornada" (asks for horaInicio/horaFin) and "agregar curso" (jornada already exists, horas read-only) based on whether `buscarJornada(...)` finds one.
